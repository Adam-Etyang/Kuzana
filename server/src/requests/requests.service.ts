...
export class RequestsService {
  ...
  async createRequest(menteeId: string, mentorId: string, message?: string) {
    if (menteeId === mentorId) throw new BadRequestException('Cannot send request to yourself');

    const mentor = await this.prisma.user.findUnique({ where: { id: mentorId } });
    if (!mentor || mentor.role !== 'MENTOR') throw new BadRequestException('User is not a mentor');

    const existing = await this.prisma.mentorshipRequest.findUnique({
      where: { menteeId_mentorId: { menteeId, mentorId } },
    });
    if (existing) throw new ConflictException('Request already sent to this mentor');
  ...
  async updateRequestStatus(id: string, status: RequestStatus) {
    const existing = await this.prisma.mentorshipRequest.findUnique({ where: { id } });
    if (!existing) throw new NotFoundException('Request not found');
    if (existing.status !== 'PENDING') throw new BadRequestException('Request already processed');
  ...
  async getRequestCounts(userId: string, role: 'MENTEE' | 'MENTOR') {
    ...
    const [pending, accepted, declined] = await Promise.all([
      this.prisma.mentorshipRequest.count({ where: { ...where, status: 'PENDING' } }),
      this.prisma.mentorshipRequest.count({ where: { ...where, status: 'ACCEPTED' } }),
      this.prisma.mentorshipRequest.count({ where: { ...where, status: 'DECLINED' } }),
    ]);
    return { pending, accepted, declined, total: pending + accepted + declined };
  }
}
