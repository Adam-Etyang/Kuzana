...
export class MatchingService {
  private readonly baseUrl =
    process.env.PYTHON_SCORING_URL ?? 'http://localhost:8000';
  ...
  async runMatching() {
    ...
    // Clear stale matches so re-runs reflect current users/profiles
    ...
    console.log(
      'Mentee with profile:',
      JSON.stringify(menteeWithProfile, null, 2),
    );
    console.log(
      'Mentor with profile:',
      JSON.stringify(mentorWithProfile, null, 2),
    );
